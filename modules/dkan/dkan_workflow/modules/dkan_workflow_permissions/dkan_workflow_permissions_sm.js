(function ($) {
  ...
  attach: function(context, settings) {
    var rolePairings = settings.dkan_workflow_permissions.rolePairings;
      ...
      $('#edit-roleassign-roles').find('.form-checkbox').on('click', function(event) {
      ...
      function roleIsChecked(rid) {
        return $("#edit-roleassign-roles-" + rid).prop("checked");
      }
      // Check and disable the checkbox for a given role id
      function checkAndDisable(rid) {
        $("#edit-roleassign-roles-" + rid).prop('checked', true)
        .prop("disabled", true)
              ...
              .closest("form").on('submit', function(e) {
                $("#edit-roleassign-roles-" + rid).prop("disabled", false);
              });
            }
      // Un-check and enable the checkbox for a given role id
      function unCheckAndEnable(rid) {
        $("#edit-roleassign-roles-" + rid).prop('checked', false)
        .prop("disabled", false);
      }
    }
  }
