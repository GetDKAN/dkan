...
class Referencer {
  ...
  private function registerWithResourceMapper(string $downloadUrl, string $mimeType): string {
    try {
      ...
      if ($this->getResourceMapper()->register($resource)) {
      ...
      if (isset($info[0]->identifier)) {
        ...
        $stored = $this->getResourceMapper()->get($info[0]->identifier, DataResource::DEFAULT_SOURCE_PERSPECTIVE);
  ...
  private function handleExistingResource(array $info, DataResource $stored, string $mimeType): string {
    if (
      $info[0]->perspective == DataResource::DEFAULT_SOURCE_PERSPECTIVE &&
      ...
      $this->getResourceMapper()->registerNewVersion($new);
   ...
   * Private.
   */
  private function getResourceMapper(): ResourceMapper {
  ...
  public static function hostify(string $resourceUrl): string {
    ...
    $serverHost = $serverPublicFilesUrl['host'] ?? \Drupal::request()
      ->getHost();
