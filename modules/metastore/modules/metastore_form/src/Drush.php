...
class Drush extends DrushCommands {
  ...
  public function __construct() {
    $this->moduleDirectory = DRUPAL_ROOT . "/" . drupal_get_path("module", "metadata_form");
    $this->librariesFilePath = $this->moduleDirectory . "/metadata_form.libraries.yml";
  ...
  private function setLibraries($libraries, $chunks, $type, $base) {
    foreach ($chunks as $chunk) {
      if ($type == 'js') {
        $libraries['metadata_form']['js'][$base . $chunk] = [];
      }
      else {
        $libraries['metadata_form']['css']['base'][$base . $chunk] = [];
      }
    }
    return $libraries;
  }
