...
class DataDictionaryWidget extends WidgetBase implements TrustedCallbackInterface {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    $element['dictionary_fields'] = [
      ...
      '#suffix' => '</div>',
      '#markup' => $this->t('<div class="claro-details__description">A data dictionary for this resource, compliant with the <a href="https://specs.frictionlessdata.io/table-schema/" target="_blank">Table Schema</a> specification.</div>'),
      '#pre_render' => [
      [$this, 'preRenderForm'],
      ],
    ];
    ...
    foreach ($data_results as $key => $data) {
      if (isset($op_index[0]) && in_array($op_index[0], $action_list) && array_key_exists($key, $fields_being_modified)) {
        ...
        $element['dictionary_fields']['edit_fields'][$key]['format'] = [
          ...
          '#required' => TRUE,
          '#title' => 'Format',
          '#description' => $this->t(DataDictionary::generateFormatDescription($current_fields[$key]['type'])),
        ...
        $element['dictionary_fields']['edit_fields'][$key]['format_other'] = [
          ...
          // '#required' => TRUE,
          '#value' => $current_fields[$key]['format_other'] ?? NULL,
        ...
        $element['dictionary_fields']['edit_fields'][$key]['description'] = [
          ...
          '#required' => TRUE,
          '#title' => 'Description',
          '#description' => $this->t('Information about the field data.'),
        ];
  ...
  public function updateFormatOptions(array &$form, FormStateInterface $form_state) {
    ...
    $format_field['#description'] = $this->t(DataDictionary::generateFormatDescription($data_type));
      ...
      $add_fields = [
        ...
        'group' => [
          ...
          'format' => [
            ...
            '#required' => TRUE,
            '#title' => 'Format',
            '#description' => $this->t(DataDictionary::generateFormatDescription("string")),
